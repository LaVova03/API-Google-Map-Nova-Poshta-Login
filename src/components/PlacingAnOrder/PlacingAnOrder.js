import './PlacingAnOrder.css';
import { useState } from 'react';
import { API_URL, API_KEY } from '../../constants/Constants';
import Map from '../../components/PlacingAnOrder/GoogleMap/GoogleMap';
...
const PlacingAnOrder = () => {
    ...
    const [itemNp, setItemNp] = useState([])
        ...
        if (city) {
            try {
                ...
                if (response.ok) {
                    ...
                    setItemNp(data.data[0].ShortAddress)
                }
            } catch {
                console.log('Error with fetch setRegion')
            }
